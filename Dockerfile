...
# RUN apk upgrade
RUN apk --no-cache add linux-headers build-base curl gettext pkgconf
RUN apk --no-cache add libstdc++6 libc6-compat
# RUN apk --no-cache add zlib-static bzip2-static

RUN apk --no-cache add gawk m4 gcc6 g++6 git zip unzip perl cmake ninja
#for libneon build
RUN apk --no-cache add bzip2 xz patch wget which autoconf automake libtool gettext xmlto
...
RUN rm -rf $PREFIX/CMakeCache.txt 2>&1 && \
  ...
  -DCMAKE_EXE_LINKER_FLAGS="-fPIC -Os -static-libstdc++ -static -pthread" \
